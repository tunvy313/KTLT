...
class themkhoanchi_ext(Ui_themkhoanchi_form):
    PATH="data/khoanchi.json"


    def setupUi(self, themkhoanchi_form):
        super().setupUi(themkhoanchi_form)
        self.themkhoanchi = themkhoanchi_form
    ...
    def insert_data (self):
        ...
        if not tien.replace('.', '', 1).isdigit():  # Cho phép số có dấu chấm (số thực)
            QMessageBox.warning(self.themkhoanchi, "Lỗi", "Số tiền phải là một số hợp lệ!")
            return
        ...
        if tien and ngay and khoanchi:
            ...
            QMessageBox.warning(self.themkhoanchi, "Lỗi", "Hãy nhập đầy đủ thông tin giao dịch.")


    def update_data(self):
        selected_row = self.tbl_table.currentRow()
        if selected_row >= 0:
                reply = QMessageBox.question(self.themkhoanchi, "Xác nhận", "Bạn chắc chắn muốn cập nhật giao dịch này",
    ...
    def delete_data(self):
        selected_row = self.tbl_table.currentRow()
        if selected_row >= 0:
            reply = QMessageBox.question(self.themkhoanchi, "Thông báo", "Bạn có chắc chắn muốn xóa giao dịch",
                                         QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
    ...
    def process_quaylai(self):
        ...
        self.themkhoanchi.close()  # Đóng cửa sổ hiện tại
        self.manhinhchinh_form.show()  # Hiển thị màn hình chính

    #Tính tổng khoản chi
    def tinh_tong_so_tien(self):
        tong_tien = 0
        so_hang = self.tbl_table.rowCount()

        for row in range(so_hang):
            item = self.tbl_table.item(row, 0)  # Cột 0 là "Số tiền"
            if item and item.text().strip():  # Kiểm tra nếu có dữ liệu
                try:
                    tong_tien += int(item.text())
                except ValueError:
                    print(f"Lỗi chuyển đổi ở hàng {row}: {item.text()}")
        QMessageBox.information(self.themkhoanchi, "Tổng số tiền đã chi", f"Tổng số tiền: {tong_tien}")
