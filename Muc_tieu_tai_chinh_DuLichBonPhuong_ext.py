...
from Muc_tieu_tai_chinh_DuLichBonPhuong import Ui_DuLichBonPhuong
class Ui_DuLichBonPhuong_Ext(Ui_DuLichBonPhuong):
    def setupUi(self, du_lich_bon_phuong_mw):
        super().setupUi(du_lich_bon_phuong_mw)
        self.du_lich_bon_phuong_mw = du_lich_bon_phuong_mw
    ...
    def xem_lai_cac_quy(self):
        ...
        self.du_lich_bon_phuong_mw.close()

    def quay_lai_tu_du_lich_bon_phuong(self):
    ...
    def kiem_tra_ngay(self):
        ...
        if not ngay_bat_dau.isValid() or not ngay_ket_thuc.isValid():
            QMessageBox.information(self.du_lich_bon_phuong_mw, "Thông báo", "Ngày không hợp lệ.")
            return False

        if ngay_ket_thuc < ngay_bat_dau:
            QMessageBox.information(self.du_lich_bon_phuong_mw, "Thông báo", "Ngày kết thúc phải sau ngày bắt đầu.")
            return False

        if ngay_bat_dau < ngay_hom_nay:
            QMessageBox.information(self.du_lich_bon_phuong_mw, "Thông báo", "Ngày bắt đầu phải từ ngày hôm nay trở đi.")
            return False
    ...
    def kiem_tra_ten(self):
        if self.txtTenQuy.text() == "":
            QMessageBox.warning(self.du_lich_bon_phuong_mw, "Cảnh báo", "Hãy điền tên quỹ")
            return False
        return True
