...
from Muc_tieu_tai_chinh_SinhHoatHangNgay import Ui_SinhHoatHangNgay
class Ui_SinhHoatHangNgay_Ext(Ui_SinhHoatHangNgay):
    def setupUi(self, sinh_hoat_hang_ngay_mw):
        super().setupUi(sinh_hoat_hang_ngay_mw )
        self.sinh_hoat_hang_ngay_mw = sinh_hoat_hang_ngay_mw
    ...
    def xem_lai_cac_quy(self):
        ...
        self.sinh_hoat_hang_ngay_mw.close()

    #còn code bấm xác nhận, xoá cửa sổ khi trở lại ầânf hình chính, và code 5 ô còn lại
    def quay_lai_tu_sinh_hoat_hang_ngay(self):
    ...
    def kiem_tra_ngay(self):
        ...
        if not ngay_bat_dau.isValid() or not ngay_ket_thuc.isValid():
            QMessageBox.information(self.sinh_hoat_hang_ngay_mw, "Thông báo", "Ngày không hợp lệ.")
            return False

        if ngay_ket_thuc < ngay_bat_dau:
            QMessageBox.information(self.sinh_hoat_hang_ngay_mw, "Thông báo", "Ngày kết thúc phải sau ngày bắt đầu.")
            return False

        if ngay_bat_dau < ngay_hom_nay:
            QMessageBox.information(self.sinh_hoat_hang_ngay_mw, "Thông báo",
    ...
    def kiem_tra_ten(self):
        if self.txtTenQuy.text() == "":
            QMessageBox.warning(self.sinh_hoat_hang_ngay_mw, "Cảnh báo", "Hãy điền tên quỹ")
            return False
        return True
