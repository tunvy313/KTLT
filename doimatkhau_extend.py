...
class doimatkhau_ext(QMainWindow, Ui_doimatkhau_form):
    def setupUi(self, doimatkhau_form):
        ...
        self.btnanmatkhaucu.clicked.connect(self.hienmatkhaucu)
        self.btnanmatkhaumoi.clicked.connect(self.hienmatkhaumoi)
        self.btn_return.clicked.connect(self.back_to_quenmatkhau_form)
        self.btndangky.clicked.connect(self.back_to_dangky)

    def hienmatkhaucu(self):
        """Hiện/Ẩn mật khẩu cũ"""
        if self.txtmatkhaumoi.echoMode() == QLineEdit.EchoMode.Password:
            self.txtmatkhaumoi.setEchoMode(QLineEdit.EchoMode.Normal) # Hiển mật khẩu
        else:
            self.txtmatkhaumoi.setEchoMode(QLineEdit.EchoMode.Password) # Ẩn lại mật khẩu

    def hienmatkhaumoi(self):
        """Hiện/Ẩn mật khẩu mới"""
        if self.txtnhaplaimatkhaumoi.echoMode() == QLineEdit.EchoMode.Password:
            self.txtnhaplaimatkhaumoi.setEchoMode(QLineEdit.EchoMode.Normal)
        else:
            self.txtnhaplaimatkhaumoi.setEchoMode(QLineEdit.EchoMode.Password)
